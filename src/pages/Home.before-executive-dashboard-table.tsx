...
const exposureRows = [
  {
    ...
    waitingOn: 'Final structural connection details',
    consequence: 'Steel fabrication cannot begin',
    exposure: 'Critical',
    tone: 'critical',
  },
  {
    item: 'Custom Steel Doors & Windows',
    waitingOn: 'Final frame and glazing approval',
    consequence: 'Release delayed • Fabrication window missed',
    exposure: 'High',
    tone: 'critical',
  },
  {
    item: 'HVAC',
    waitingOn: 'Mechanical room coordination complete',
    consequence: 'Equipment submittal and fabrication delayed',
    exposure: 'High',
    tone: 'high',
  },
  {
    item: 'Electrical Switch Gear',
    waitingOn: 'Utility requirements and electrical approval',
...
function MarginExposureBadge({ tone, children }: { tone: string; children: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs font-semibold ${classes}`}>
            ...
            <div className="rounded-xl border border-slate-700/70 bg-slate-950/90">
              ...
              <div className="overflow-x-auto">
                <div className="min-w-[900px]">
                  <div className="grid grid-cols-[28%_30%_28%_14%] border-b border-slate-800 text-sm text-slate-400">
                    <div className="px-5 py-3 font-medium">Procurement Package</div>
                    <div className="px-5 py-3 font-medium">Waiting On</div>
                    <div className="px-5 py-3 font-medium">Next Consequence</div>
                    <div className="px-5 py-3 font-medium">Margin Exposure</div>
                  </div>

                  <div className="divide-y divide-slate-800">
                    {exposureRows.map((row) => (
                      <div key={row.item} className="grid grid-cols-[28%_30%_28%_14%] items-center text-sm">
                        <div className="px-5 py-4 font-medium text-slate-100">{row.item}</div>
                        <div className="px-5 py-4 text-slate-400">{row.waitingOn}</div>
                        <div className="px-5 py-4 text-slate-300">{row.consequence}</div>
                        <div className="px-5 py-4"><MarginExposureBadge tone={row.tone}>{row.exposure}</MarginExposureBadge></div>
                      </div>
                    ))}
                  </div>
                </div>
