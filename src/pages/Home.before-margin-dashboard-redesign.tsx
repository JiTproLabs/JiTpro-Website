...
const exposureRows = [
  {
    item: 'Cabinet package',
    owner: 'Owner selection',
    needed: '14 days',
            ...
            <div className="rounded-xl border border-slate-700/70 bg-slate-950/90">
              <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
                <div>
                  ...
                  <h2 className="mt-1 font-heading text-xl font-700 text-slate-100">Decision-to-delivery exposure</h2>
                </div>
                <StatusBadge tone="attention">3 items need attention</StatusBadge>
              </div>

              <div className="grid border-b border-slate-800 text-sm text-slate-400 md:grid-cols-[1.15fr_1fr_0.65fr_0.8fr]">
                <div className="px-5 py-3 font-medium">Package</div>
                <div className="hidden px-5 py-3 font-medium md:block">Waiting on</div>
                <div className="hidden px-5 py-3 font-medium md:block">Needed in</div>
                <div className="hidden px-5 py-3 font-medium md:block">Status</div>
              </div>

              <div className="divide-y divide-slate-800">
                {exposureRows.map((row) => (
                  <div key={row.item} className="grid gap-3 px-5 py-4 text-sm md:grid-cols-[1.15fr_1fr_0.65fr_0.8fr] md:items-center md:gap-0 md:px-0">
                    <div className="font-medium text-slate-100 md:px-5">{row.item}</div>
                    <div className="text-slate-400 md:px-5">{row.owner}</div>
                    <div className="font-mono text-slate-300 md:px-5">{row.needed}</div>
                    <div className="md:px-5"><StatusBadge tone={row.tone}>{row.status}</StatusBadge></div>
                  </div>
                ))}
              </div>
            </div>
