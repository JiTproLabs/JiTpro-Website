/**
 * Commitment Register - representative fixture data.
 *
 * Approved as part of Visual Master v1. Content lives here rather than in JSX
 * so the table and the detail panel cannot drift apart: both read this file.
 *
 * PRODUCT MODEL. `responsibleOrg` is the organization or project entity that
 * OWES the commitment; `ownerName` is the specific person accountable for
 * resolving it. There is deliberately NO financial field - no value, cost,
 * budget, or contract value - because JiTpro does not manage those, and none
 * may be reintroduced here.
 *
...
/**
 * Pagination, reproduced as the reference renders it. The reference is
 * internally inconsistent ("1 to 8 of 23" with "Rows per page: 25" across five
 * pages); Visual Master v1 keeps that rather than silently correcting it.
 */
