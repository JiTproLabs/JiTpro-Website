...
| | |
|---|---|
| Status | **Sprint 1a complete and CI-verified on draft PR #52 (2026-09-13). Sprint 1b waits on the approved 31-page PDF (L-8). Sprint 2 targets `jitpro_website`, the website's only Supabase project (G-8, D5.11 as amended 2026-09-14). The three approved migrations were **applied to `jitpro_website` on 2026-09-14 and verified** (break-point record under Sprint 2 in §22). **Paused on one verification finding:** the identity sequence `lead_magnet_ip_activity_id_seq` still grants anon and authenticated privileges; the fix, migration `20260914000004`, is proposed and awaits Jeff's approval. No secret set, no function deployed, no existing object changed. Dependencies are Dependabot-only (G-6).** |
| Owner / approver | Jeff Kaufman |
| Document created | 2026-09-12 |
| Last updated | 2026-09-15 (break-point record: Sprint 2 migrations applied and verified; sequence-privilege finding and proposed fix `20260914000004` awaiting approval) |
...
| Resume point | Jeff's decision on `20260914000004`. After it is applied and verified: record the final result here, then bring Jeff the complete `submit-lead-magnet-request` proposal required by §15.2 before any deployment. |
