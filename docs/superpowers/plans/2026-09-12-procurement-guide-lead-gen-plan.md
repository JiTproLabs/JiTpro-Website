...
4. **Resolve the missing funnel events from that manual test.** ~~Read-only diagnosis.~~ **Done 2026-09-16: root cause established (the `sendBeacon` transport's credentialed preflight fails against a CORS helper that grants no credentials, so no event POST is ever sent; record below). Client-only fix and test plan PROPOSED, awaiting Jeff's approval; then implement, test, CI.** PR #52 is not marked ready until the fix is verified.
5. Mark PR #52 ready **on Jeff's instruction**.
6. **Jeff's explicit merge approval**, then the squash merge (squash-only ruleset; `build-and-test` required and strict).
7. Verify the Cloudflare production deployment.
8. Run the live smoke test, verify the email and the analytics events.
9. Propose the guarded cleanup of the smoke-test rows.
10. Record the launch in this plan.
