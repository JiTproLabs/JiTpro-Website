...
| | |
|---|---|
| Status | **Sprint 1a complete and CI-verified on draft PR #52 (2026-09-13). Sprint 1b waits on the approved 31-page PDF (L-8). Sprint 2 targets `jitpro_website`, the website's only Supabase project (G-8, D5.11 as amended 2026-09-14). All four Sprint 2 migrations are **applied to `jitpro_website` and verified** (2026-09-14 and 2026-09-15, records under Sprint 2 in §22), including the sequence-privilege fix `20260914000004`. The `submit-lead-magnet-request` proposal (§15.2) is next and goes to Jeff before any deployment. No secret set, no function deployed, no existing object changed. Dependencies are Dependabot-only (G-6).** |
| Owner / approver | Jeff Kaufman |
| Document created | 2026-09-12 |
| Last updated | 2026-09-15 (sequence fix `20260914000004` applied and verified; S2-10 rule added; Sprint 2 database work complete) |
